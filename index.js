...
if (argv.from && argv.to) {
  compareReports(
	...
	// Create report directory if it does not exists
  ...
  launchChromeAndRunLighthouse(argv.url).then(results => {
    const prevReports = glob(`${dirName}/*.json`, {
      sync: true
		...
		// Check if we have previously run a report for the site, if we
		// have then run the comparison.
    if (prevReports.length) {
      ...
      compareReports(recentReportContents, results.js);
		...
		// Write the report to disk adding a timestamp
    fs.writeFile(
